...
export function RestaurantSettings() {
    ...
    // Takeout Timing State
    const [takeoutTiming, setTakeoutTiming] = useState<'before' | 'after'>('after');
    const [takeoutTimingChanged, setTakeoutTimingChanged] = useState(false);

    useEffect(() => {
        if (restaurant) {
            setName(restaurant.name || '');
            setTakeoutTiming(restaurant.takeoutPaymentTiming || 'after');
    ...
    useEffect(() => {
        if (restaurant) {
            ...
            setTakeoutTimingChanged(takeoutTiming !== (restaurant.takeoutPaymentTiming || 'after'));
    ...
    }, [name, paymentMethods, restaurant, takeoutTiming]);
    ...
    const handleSaveTakeoutTiming = async () => {
        if (!restaurant) return;
        setLoading(true);
        try {
            await updateDoc(doc(db, 'restaurants', restaurant.id), {
                takeoutPaymentTiming: takeoutTiming,
                updatedAt: Date.now()
            });
            toast.success("Configuración actualizada");
            setTakeoutTimingChanged(false);
        } catch (e) {
            console.error(e);
    ...
    const handleQrUpload = async (file: File, methodId: string) => {
        ...
        try {
            ...
            const options = {
                maxSizeMB: 0.1, // Max 100KB is plenty for a QR
            ...
            reader.onloadend = () => {
                ...
                setPaymentMethods(prev => prev.map(p =>
                    p.id === methodId ? { ...p, qrUrl: base64data } : p
                ));
                setUploadingMethodId(null);
                toast.success("QR cargado correctamente");
            };
        } catch (error) {
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {!restaurant ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6 h-fit">
                        {/* Delivery/Takeout Flow Config */}
                        <div className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                    <span className="text-orange-500">🛍️</span> Pedidos Para Llevar
                                </h3>
                                {/* Granular Save Button for Takeout Timing */}
                                {takeoutTimingChanged && (
                                    <button
                                        onClick={handleSaveTakeoutTiming}
                                        disabled={loading}
                                        className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded-lg font-bold text-sm shadow-lg animate-in fade-in transition-all"
                                    >
                            ...
                            <div>
                                <p className="text-sm text-slate-400 mb-3">¿En qué momento se cobran los pedidos para llevar?</p>
                                <div className="flex flex-col gap-3">
                                    <label className={clsx(
                                        ...
                                        <input
                                            type="radio"
                                            name="paymentTiming"
                                            checked={takeoutTiming === 'after'}
                                            onChange={() => setTakeoutTiming('after')}
                                            className="w-4 h-4 text-blue-500"
                                        />
                                        <div>
                                            <span className="block text-white font-medium">Cobrar al Entregar (Recomendado)</span>
                                            <span className="text-xs text-slate-500">El pedido va a cocina inmediatamente. Se cobra al finalizar.</span>
                                        </div>
                                    </label>

                                    <label className={clsx(
                                        "flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all",
                                        takeoutTiming === 'before'
                                            ? "bg-slate-800 border-blue-500 ring-1 ring-blue-500"
                                            : "bg-slate-950 border-slate-700 hover:border-slate-500"
                                    )}>
                                        <input
                                            type="radio"
                                            name="paymentTiming"
                                            checked={takeoutTiming === 'before'}
                                            onChange={() => setTakeoutTiming('before')}
                                            className="w-4 h-4 text-blue-500"
                                        />
                                        <div>
                                            <span className="block text-white font-medium">Cobrar Antes (Pre-pago)</span>
                                            <span className="text-xs text-slate-500">Se exige el pago antes de poder enviar la orden a cocina.</span>
                                        </div>
                                    </label>
                                </div>
                            </div>
                    ...
                    {/* 3. Payment Methods */}
                    <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 h-fit lg:col-span-2">
